...
export const mockContacts: Contact[] = [
    { id: 1, name: 'ישראל ישראלי', phone: '054-1234567', mobilePhone: '050-1112222', email: 'israel@getter.co.il', role: 'מנהל גיוס', linkedin: 'https://linkedin.com/in/israel', username: '', isActive: true, notes: 'איש קשר ראשי לכל המשרות הטכנולוגיות.', isInvited: true, hasSystemAccess: true },
    { id: 2, name: 'דנה כהן', phone: '052-7654321', mobilePhone: '050-3334444', email: 'dana@getter.co.il', role: 'רכזת גיוס', linkedin: 'https://linkedin.com/in/dana', username: '', isActive: true, notes: 'לתאם מולה ראיונות טכניים.', isInvited: false, hasSystemAccess: false },
    { id: 3, name: 'אבי לוי', phone: '050-9876543', mobilePhone: '050-5556666', email: 'avi@getter.co.il', role: 'מנהל תיק לקוח', linkedin: 'https://linkedin.com/in/avi', username: '', isActive: false, notes: 'לא פעיל כרגע.', isInvited: true, hasSystemAccess: false },
];
...
const ClientContactsTab: React.FC<ClientContactsTabProps> = ({ clientId, onOpenMessageModal }) => {
    ...
    const handleSystemAccessToggle = (e: React.SyntheticEvent, contactId: number) => {
        e.stopPropagation();
        setContacts(prev => prev.map(c => c.id === contactId ? { ...c, hasSystemAccess: !c.hasSystemAccess } : c));
    };

    const handleInviteToSystem = (e: React.SyntheticEvent, contactId: number) => {
        e.stopPropagation();
        setContacts(prev => prev.map(c => c.id === contactId ? { ...c, isInvited: true, hasSystemAccess: true } : c));
        alert('הזמנה למערכת נשלחה בהצלחה');
    };

    const handleResetPassword = (e: React.SyntheticEvent, contactId: number) => {
        e.stopPropagation();
