...
const EventsView: React.FC<EventsViewProps> = ({ events, setEvents }) => {
    ...
    const handleSaveEvent = (eventData: Omit<Event, 'id' | 'status' | 'linkedTo'> & { id?: number }) => {
        if (eventData.id) {
            ...
            const newEvent: Event = {
                ...
                coordinator: 'אני', 
                status: 'עתידי',
                linkedTo: { type: 'מועמד', name: 'גדעון שפירא' },
    ...
    const renderCell = (event: Event, columnId: string) => {
        switch (columnId) {
            ...
            case 'linkedTo': return event.linkedTo ? <a href="#" onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 text-primary-600 hover:underline"><LinkIcon className="w-4 h-4"/>{event.linkedTo.name}</a> : <span className="text-text-subtle">-</span>;
